...
var EventDispatcher = (function() {	
	...
	EventDispatcher.prototype = {
		...
		visitor: function(visitor) {
			if( arguments.length <= 0 ) return this._visitor;
		...
		scope: function(scope) {
			if( arguments.length <= 0 ) return this._scope || this;
		...
		source: function(source) {
			if( arguments.length <= 0 ) return this._source || this;
		...
		on: function(actions, fn, options) {
			if( typeof(actions) !== 'string' ) throw new Error('invalid action name:' + actions);
			...
			if( options.bubble !== true ) options.bubble = false;
			
			var listeners = this.listeners();
			var arg = actions.split(' ');
			for(var i=0; i < arg.length; i++) {
				var action = arg[i];
				if( !action ) continue;
				if( !listeners[action] ) listeners[action] = [];

				var listener = listeners[action];

				// 이미 존재한다면 삭제
				var index = listener.indexOf(fn);
				if( index >= 0 ) {
					listener[index] = null;
					try { delete listener[index]; } catch(e) {}
				}
				
				if( options.noaddifexist && listener.length > 0 ) return this;

				fn.__bubble__ = options.bubble;
				listener.push(fn);
				this.fireSync('event.on', {
					eventType: action,
					eventHandler: fn,
					eventBubble: options.bubble
				});
			}

			return this;
		},
		un: function(actions, fn, bubble) {
			if( typeof(actions) !== 'string' ) return this;
			if( !fn || !(typeof(fn) === 'object' || typeof(fn) === 'function') ) return this;

			var listeners = this.listeners();
			var arg = actions.split(' ');
			for(var i=0; i < arg.length; i++) {
				var action = arg[i];
				if( !action || !listeners[action] ) continue;

				for(var i=0;i < fns.length;i++) {
					if( fns[i] === fn && bubble === fns[i].__bubble__) {
		...
		dispatchEvent: function(event, scope) {
			...
			var fns = global.concat(listeners[event.type] || []);
			...
			if( fns && fns.length > 0 ) {
				for(var i=(fns.length - 1);i >= 0;i--) {
					...
					var result;
					if( typeof(fn) == 'function' ) {
						result = fn.call(scope || {}, event);
					} else if( typeof(fn) == 'object' && typeof(fn.handleEvent) === 'function' ) {						
						result = fn.handleEvent(event);
					} else {
						console.warn('invalid event listener(bypassed)', fn.toString());
					}
...
console.log('name.url(file)', Path.name(url_file));
console.log('name.windows(file)', Path.name(windows_file));
console.log('name.unix(file)', Path.name(unix_file));

console.log('name.url(dir)', Path.name(url_dir));
console.log('name.windows(dir)', Path.name(windows_dir));
console.log('name.unix(dir)', Path.name(unix_dir));
	...
	Ajax.prototype = {		
		...
		ajax: function(url) {
			...
			if( typeof(url) === 'object' ) {
				...
				throw new Error('illegal ajax option(string or object):' + url);
			}
					...
					listeners: {done:[],success:[],error:[]},
						...
						if( l ) {
							l.forEach(function(item) {
								...
								else throw new Error('complete listener is not a function.');
							});
						}
					},
					success: function() {
						...
						if( l ) {
							l.forEach(function(item) {
								if( typeof(item) === 'function' ) item.apply(o.scope || item, args);
								else throw new Error('success listener is not a function.');
							});
						}
					},
					error: function() {
						...
						if( l ) {
							l.forEach(function(item) {
								if( typeof(item) === 'function' ) item.apply(o.scope || item, args);
								else throw new Error('error listener is not a function.');
							});
						}
					},
					commit: function() {
	...
	function createRequire(src) {
		...
		return (function(base) {
			return function require(path, reload, nocache) {
				if( Require.bundle(path) ) return Require.get(path, reload, nocache);

				var url = Path.join(base, path);
				return Require.get(url, reload, nocache);
	...
	return Require = (function() {
			...
			try {
				if( window.JSON ) {
					try {
						...
						console.error('[WARN] json [' + src + '] has instrict syntax(' + err.message + ').', err);
						throw new SyntaxError('json file [' + src + '] has syntax error (' + e.message + ')');
					}
				} else {
					eval('json = ' + json);
				}
		...
		function conn(src, cache, sync, fn) {
			Ajax.ajax(src).cache(cache).parse(false).sync(sync).get().done(function(err, data, xhr) {
				...
				if( ~xhr.getResponseHeader('content-type').indexOf('/json') || (src.lastIndexOf('.json') >= 0 && src.lastIndexOf('.json') === src.length - '.json'.length) ) {
					data = {
						exports: eval_json(data, src)
					};
				} else if( ~xhr.getResponseHeader('content-type').indexOf('text/') || (src.lastIndexOf('.html') >= 0 && src.lastIndexOf('.html') === src.length - '.html'.length) ) {
					data = data;
				} else {
		...
		Require.prototype = {
			...
			get: function(path, reload, nocache) {
				...
				if( typeof(bundle) === 'function' ) {
					module = cached[path] || eval_as_module(bundle, path);
				} else if( bundle ) {
					module = {exports:bundle};
				} else {
					module = (cached[path]) ? cached[path] : this.sync(path, nocache);
				}
			...
			sync: function(src, nocache) {
				var module;
				var cache = (nocache === true) ? false : true;
				conn(src, cache, true, function(err, data) {
					if( err ) throw err;
			...
			async: function(src, nocache) {
				return {
					done: function(fn) {
						conn(src, nocache, false, fn);
					}
				};
			}
		};
