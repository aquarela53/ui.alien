 ...
 * 
 * Lisence:
 *		under MIT Lisence (https://github.com/joje6/attrs.oop/blob/master/LICENSE)
 */
 (function() {
	var global = this;
	...
	Class.define = function(name, c) {
		...
		if((pos = name.lastIndexOf('.')) > 0) {
			...
			cls.__class__.initializerName = name;
		}
	...
	Class.copy = function(target, source, attrs) {
		...
		for(var k in source) {
			...
			if ( (g || s) ) {
				...
				//if( source[k] == this.initializer ) continue;
	...
	Protocol.prototype = {
		validation: function(cls) {
			...
			if( typeof(protocol) == 'function' ){
				if( !(protocol.apply(protocol, [this])) ) throw new TypeError('Class [' + name + '] is invalid according to protocol [' + pname + '].');
			} else {
				for(var k in protocol) {
					...
					if( !m ) {
						...
						if( !d.apply(protocol, [m, this]) ) throw new TypeError('method [' + k + '] invalid. according to [' + this.name + ']');
					} else if( d !== true && d >= 0 && m.length !== d ) {
